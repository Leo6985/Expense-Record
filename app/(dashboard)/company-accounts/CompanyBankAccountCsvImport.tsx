...
import { importCompanyBankAccountsCSV } from "@/actions/payments";

type AccountRow = { bankName: string; branch?: string; accountNo: string; accountName: string };
type ImportResult = { created: number; updated: number; errors: string[] };

const HEADERS = ["bankName", "branch", "accountNo", "accountName"];
...
function rowsFromObjects(objects: Record<string, unknown>[]): AccountRow[] {
  return objects.map((obj) => {
    ...
    return {
      ...
      branch: mapped["branch"] || undefined,
...
export default function CompanyBankAccountCsvImport() {
  ...
  async function handleImport() {
    ...
    setLoading(true);
    try {
      const res = await importCompanyBankAccountsCSV(rows);
      setResult(res);
      setRows([]);
    } finally {
      setLoading(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-900">นำเข้า / ส่งออกบัญชีธนาคารบริษัท</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3 flex-wrap pt-2 border-t border-gray-100">
                ...
                <input
                  ...
                  onChange={handleFile}
                />
                <span className="text-xs text-gray-400">รองรับ .csv (UTF-8) และ .xlsx · ถ้าเลขบัญชีซ้ำจะอัปเดตข้อมูล</span>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg px-4 py-3 text-xs text-gray-500">
                ...
                <code className="font-mono">bankName, branch, accountNo, accountName</code>
              </div>
                    ...
                    <table className="text-xs w-full">
                      <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                          {["ธนาคาร", "สาขา", "เลขบัญชี", "ชื่อบัญชี"].map((h) => (
                      ...
                      <tbody>
                        {rows.slice(0, 10).map((r, i) => (
                          <tr key={i} className="border-b border-gray-100">
                            ...
                            <td className="px-3 py-1.5 text-gray-500">{r.branch || "-"}</td>
                            <td className="px-3 py-1.5 font-mono">{r.accountNo}</td>
                            <td className="px-3 py-1.5">{r.accountName}</td>
                          </tr>
                        ))}
                        {rows.length > 10 && (
                          <tr><td colSpan={4} className="px-3 py-2 text-center text-gray-400">... และอีก {rows.length - 10} แถว</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
