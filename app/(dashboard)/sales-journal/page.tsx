...
import {
  ...
  SalesJournalView,
  GenerateSalesVouchersResult,
} from "@/actions/sales-journal";
...
export default function SalesJournalPage() {
  ...
  const [view, setView] = useState<SalesJournalView | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [generating, setGenerating] = useState(false);
  const [result, setResult] = useState<GenerateSalesVouchersResult | null>(null);
  ...
  async function handleSearch() {
    ...
    setError(null);
    try {
      setView(await getSalesJournal({ from, to }));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const res = await generateSalesVouchers({ from: view.from, to: view.to });
      setResult(res);
      setView(await getSalesJournal({ from: view.from, to: view.to }));
    } catch (e) {
  ...
  function handleDownloadCSV() {
    if (!view) return;
    const headers = ["วันที่", "เลขที่ใบกำกับ", "ลูกค้า", "เดบิต ลูกหนี้", "เครดิต รายได้", "เครดิต ภาษีขาย", "เลขที่ Voucher", "สถานะ Voucher"];
    ...
    const t = view.totals;
    rows.push(["", "", "รวม", t.debitAR, t.creditRevenue, t.creditVat, "", ""]);
    downloadCSV(`sales_journal_${view.from}_${view.to}.csv`, headers, rows);
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">สมุดรายวันขาย</h1>
          <p className="text-gray-500 text-sm mt-0.5">
            สร้างใบสำคัญรายวัน (Voucher) อัตโนมัติจากใบกำกับภาษีขาย — หนึ่งใบกำกับต่อหนึ่ง Voucher
          </p>
        </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-gray-600">
              ใบกำกับภาษีขาย {view.rows.length} ใบ · ยังไม่มี Voucher{" "}
          ...
          {view.rows.length === 0 ? (
            <div className="text-center text-gray-400 text-sm py-12 bg-white rounded-xl border border-gray-200">
              ไม่มีใบกำกับภาษีขายในช่วงเวลาที่เลือก
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200 text-gray-600">
                    ...
                    <th className="text-right px-3 py-2 font-medium border-l border-gray-200">เดบิต ลูกหนี้</th>
                    <th className="text-right px-3 py-2 font-medium">เครดิต รายได้</th>
                    <th className="text-right px-3 py-2 font-medium">เครดิต ภาษีขาย</th>
                ...
                <tbody>
                  {view.rows.map((r) => {
                    ...
                    const isExpanded = expandedId === r.invoiceId;
                    return (
                      <Fragment key={r.invoiceId}>
                        <tr className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="px-3 py-2">
                            <button
                              type="button"
                              onClick={() => setExpandedId(isExpanded ? null : r.invoiceId)}
                          ...
                          <td className="px-3 py-2 font-mono text-gray-700">{r.invoiceNumber}</td>
                          <td className="px-3 py-2 text-gray-800">{r.customerName}</td>
                          <td className="px-3 py-2 text-right text-gray-700 border-l border-gray-100">{cell(r.debitAR)}</td>
                          <td className="px-3 py-2 text-right text-gray-700">{cell(r.creditRevenue)}</td>
                          <td className="px-3 py-2 text-right text-gray-700">{cell(r.creditVat)}</td>
                ...
                <tfoot>
                  <tr className="bg-gray-50 font-semibold border-t-2 border-gray-300 text-gray-900">
                    ...
                    <td className="px-3 py-2.5 text-right border-l border-gray-200">฿{formatCurrency(view.totals.debitAR)}</td>
                    <td className="px-3 py-2.5 text-right">฿{formatCurrency(view.totals.creditRevenue)}</td>
                    <td className="px-3 py-2.5 text-right">฿{formatCurrency(view.totals.creditVat)}</td>
